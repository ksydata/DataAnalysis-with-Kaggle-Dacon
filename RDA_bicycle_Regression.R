...
summary(lm6)
  ...
  # 기존 (-) 관계가 강화되었다는 결과가 나오려면 (-)*(-) = (+)

  # H6 : humidity -> total (-)에 대하여 working의 조절효과가 존재한다.(b6가 양수인지 음수인지 미지정 -> 양측검정) 
  # b6 = 1.951 양수로 유의한바 기존 인과관계 약화
  # 즉, 습도가 높아지면 대여수 줄어들지만, 일하는 시기(workingYes)에는 줄어드는 정도가 완화된다는 결론
